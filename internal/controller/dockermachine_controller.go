...
func (r *DockerMachineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	defer func() {
		logger.Info("Reconcile of dockermachine complete.")
	}()
	...
	// From now on, patch the DockerMachine with any changes on return from this function.
	...
	if !controllerutil.ContainsFinalizer(dockerMachine, infrav1.MachineFinalizer) {
		...
		// Return, to allow the resource to save. The next run of reconciler won't need to add finalizer again.
		return ctrl.Result{}, nil
	}
